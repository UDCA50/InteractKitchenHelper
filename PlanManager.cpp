...
PlanManager::PlanManager() {
	...
	for (unsigned int planIter = 0; planIter < 31; planIter++) {
		Day myDate;
...
void PlanManager::saveMonthPlan() {
	...
	while (date < 32) {
		...
		for ( int planIter = 0; planIter < saveDay->getPlanCount(); planIter++) {
			fprintf(datefptr, ",%s", savePlanData[planIter].c_str());
		}
		fprintf(datefptr, "\n");
		date++;
	}
